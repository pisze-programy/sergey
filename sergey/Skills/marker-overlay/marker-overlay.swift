...
class MarkerOverlayExecutor: SkillExecutor {
    func execute(params: [String: Any]) async throws -> Any {
        let manager = MarkerOverlayManager.shared
        
        // Ensure window is showing
        manager.show()
        ...
        if let rectDict = params["rect"] as? [String: Any],
           ...
           let h = rectDict["h"] as? CGFloat {
            ...
            manager.drawRect(CGRect(x: x, y: y, width: w, height: h), color: color, duration: duration)
        ...
        if let text = params["text"] as? String {
            ...
            let color = parseColor(params["color"])
            let duration = params["duration"] as? TimeInterval
            manager.drawText(text, at: CGPoint(x: x, y: y), color: color, duration: duration)
        ...
        if let pointDict = params["point"] as? [String: Any],
           ...
           let y = pointDict["y"] as? CGFloat {
            ...
            manager.moveCursor(to: CGPoint(x: x, y: y), duration: d)
